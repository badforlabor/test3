...
void PushBuffer(std::vector<char>& outRet, BYTE* src, DWORD length)
{
	for (int i = 0; i < length; i++)
		outRet.push_back(src[i]);
}
int MP3decoder::GetBytes(std::vector<char>& outRet, char* pcfilename)
{
	int ntmp = 0;
	...
	int length = 0;
	int total = 0;
	while ((length = Decode(buffer)) > 0)
	{
		total += length;
		//PushBuffer(outRet, buffer, length);
	}

	return total;// outRet.size();
}
